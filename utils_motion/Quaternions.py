...
class Quaternions:
    ...
    def transforms(self):
        ...
        """
            check column and row major!!!
        """
        #FIXME: 이거 column major랑 row major 혼동자꾸한다.. 고쳐야함!!!
        m[...,0,0] = 1.0 - (yy + zz)
        m[...,0,1] = xy - wz
        m[...,0,2] = xz + wy
        m[...,1,0] = xy + wz
        ...
        # (*, 3, 3) -> transpose 해서 column major, row major 바꿀 수 있다.
        # m = np.swapaxes(m, -2, -1)
        return m
    ...
    @classmethod
    def id(cls, n):     # 굳이 classmethod로 하는 이유가 뭐지?
